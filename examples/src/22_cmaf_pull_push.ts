...
async function runWebServer() {
  let database: Record<string, Buffer> = {};
  app.use(cors());
  app.use(express.raw({ type: ['application/vnd.apple.mpegurl', 'video/mp4', 'application/dash+xml'], limit: "5mb" }));

  app.post("*.*",
    (req: Request, res: Response) => {
      console.log("RECEIVED FILE", req.url);
